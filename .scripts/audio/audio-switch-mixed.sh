...
sink2='alsa_output.pci-0000_0b_04.0.analog-stereo'
...
# - Also set card profile
case $sink_current in
	"$sink1")
		echo "$sink1 --> $sink2"
		sink_toggle=$sink2
		pactl set-card-profile $card1 $profile2
		pactl set-source-mute $input 0;;
	"$sink2")
		echo "$sink2 --> $sink1"
		sink_toggle=$sink1
		pactl set-card-profile $card1 $profile1
...
echo pactl set-default-sink $sink_toggle

for i in $(pacmd list-sink-inputs | grep index | awk '{print $2}'); do
	pactl move-sink-input $i $sink_toggle
	echo pactl move-sink-input $i $sink_toggle
done
